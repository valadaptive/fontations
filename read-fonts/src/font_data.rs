...
use types::{BigEndian, BytesWrapper, FixedSize, LittleEndian, Scalar};
...
impl<'a> FontData<'a> {
    ...
    pub fn read_scalar_le_at<T: Scalar>(&self, offset: usize) -> Result<T, ReadError> {
        let end = offset
            .checked_add(T::RAW_BYTE_LEN)
            .ok_or(ReadError::OutOfBounds)?;
        self.bytes
    ...
    pub fn read_le_at<T: Scalar>(&self, offset: usize) -> Result<LittleEndian<T>, ReadError> {
        let end = offset
            .checked_add(T::RAW_BYTE_LEN)
            .ok_or(ReadError::OutOfBounds)?;
        self.bytes
...
impl<'a> Cursor<'a> {
    ...
    pub(crate) fn read_scalar_le<T: Scalar>(&mut self) -> Result<T, ReadError> {
    ...
    pub(crate) fn read_le<T: Scalar>(&mut self) -> Result<LittleEndian<T>, ReadError> {
